...
<body>
		...
		<table>
			...
			<tbody>
				<tr>
					...
					<td>
						<input type="password" size="14" name="m_pw" id="m_pw">
					</td>
					<td>
						<input type="text" size="14" name="m_name" id="m_name">
					</td>
					<td>
						<input type="text" size="14" name="m_age" id="m_age">
					</td>
				</tr>
			</tbody>
			<tfoot>
	...
	<script type="text/javascript">
		// ajax를 이용해서 DB 정보 가져오기 (첫 접속, 삭제, 삽입)
		function getList() {
			$.ajax({
				url : "/ajax_db_list",
				method : "post",
				...
				success : function(data) {
					...
					$(data).find("member").each(function() {
						...
						tbody += "<td>" + $(this).find("m_name").text() + "님</td>";
						tbody += "<td>" + $(this).find("m_age").text() + "살</td>";
		...
		function toggleJoinButton() {
			...
			if(isIputChk && passwordFiled && nameFiled && ageFiled) {
		...
		$("#m_id").keyup(function() {
			$.ajax({
				url : "/ajax_id_chk",
				data : "m_id=" + $("#m_id").val(), // 파라미터 (하나일 때 사용)
				method : "post",
				datatype : "text",
				success : function(data) {
					if (data == '1') {
		...
		// 파라미터 (하나일 때 사용) => data : "m_id=" + $("#m_id").val()
		// 파라미터 (여러개일 때 사용) => 보통 직렬화(serialize()) => form 태그 안에서만 가능
