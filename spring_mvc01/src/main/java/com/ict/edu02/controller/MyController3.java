...
public class MyController3 {
	...
	// @Autowired : 서비스와 같이 다른 곳에서 만들어진 객체를 가져와서 사용하기 위한 어노테이션
	// @Inject    : java
	// @Autowired : Spring
	@Autowired
	...
	/*
	@PostMapping("/calc2")
	public ModelAndView calc2Exec(CalcVO cvo) { // CalcVO에 파라미터들이 알아서 넣어진다.
	...
	*/
	...
	public ModelAndView calc2Exec(@ModelAttribute("cvo")CalcVO cvo) { 
		...
		// CalcVO에 파라미터들이 알아서 넣어진다. (변수와 파라미터 이름 같아야 한다.)
		...
		// mv.addObject("cvo", cvo);
		// 대신 40번째 줄 @ModelAttribute("cvo")CalcVO cvo) 사용
		mv.addObject("result", result);
